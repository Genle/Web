(function() {
    ...
    app.get('/new-order', (req, res) => {
       ...
       ingredients.then(
           function(data){
            ...
            res.render('neworder',{newData});
           }
       ).catch(
           function (err) {
            //    res.send(err);
            res.render('neworder', {err});
           }
       )
    ...
    app.post('/api/create/custom/pizza', (req,res) => {
        ...
        pizza.then(
            function(message){
                console.log(message);
                let order = {email:newPizza.email, pizzas:[{url:newPizza.url,title:newPizza.title,description:newPizza.description,type:1,status:newPizza.status, price:newPizza.price,email:newPizza.email}]}
            ...
            function(data){
                ...
                let order = {
                    ...
                    pizzas:[{
                        ...
                        type:data.type,
                        status:data.status,
                        price:data.price,email:data.email
                    }]}
                let orderObject = db.saveOrder(order);
