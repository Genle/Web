...
exports.tableBtn = (dateStr, index) => {
    ...
    if (seconds < 120) {
        buttons = `<td>
                    ...
                    <button id="${index}-cancel" onclick="cancelOrder(this.id) class="btn red darken-2 white-text hoverable">Cancel</button>
                    </td>`;
    } else {
        buttons = `<td>
                    <button id="${index}-reorder" onclick="orderPizza(this.id)" class="btn red darken-2 white-text hoverable">Re-order</button>
                    <button id="${index}-cancel" disabled onclick="cancelOrder(this.id)"  class="btn red darken-2 white-text hoverable">Cancel</button>
                    </td>`;
    }
...
exports.getScript = (page) => {
    ...
    switch (page) {
        ...
        case "login":
            ...
            break;
        case "signup":
            script = buildScriptTag("signupJs");
            break;
